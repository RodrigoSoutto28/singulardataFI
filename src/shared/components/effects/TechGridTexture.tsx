  ...
  return (
    ...
    >
        ...
        <defs>
          ...
          >
            <path
              d="M 32 0 L 0 0 0 32"
              fill="none"
              stroke={`hsl(var(--primary) / ${isLight ? 0.12 : 0.05})`}
              strokeWidth="1"
            />
          </pattern>
          <pattern
          ...
          >
            <path
              ...
              fill="none"
              stroke={`hsl(var(--primary) / ${isLight ? 0.18 : 0.08})`}
              strokeWidth="1"
            />
            {/* Corner ticks */}
            <path
              d="M 0 0 L 6 0 M 0 0 L 0 6"
              stroke={`hsl(var(--accent) / ${isLight ? 0.55 : 0.35})`}
              strokeWidth="1"
            />
          </pattern>
          <radialGradient id="techgrid-vignette" cx="50%" cy="50%" r="70%">
            <stop offset="0%" stopColor="hsl(var(--background))" stopOpacity="0" />
            <stop offset="100%" stopColor="hsl(var(--background))" stopOpacity={isLight ? 0.7 : 0.85} />
          </radialGradient>
        </defs>
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse 70% 50% at 0% 0%, hsl(var(--primary) / ${isLight ? 0.1 : 0.06}) 0%, transparent 60%),
            radial-gradient(ellipse 60% 45% at 100% 100%, hsl(var(--accent) / ${isLight ? 0.09 : 0.05}) 0%, transparent 55%)
          `,
        }}
      />
    </div>
